      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            {receiverTypes.map((type) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-4 p-6 rounded-2xl border transition-all duration-300 text-left group",
                    isActive 
                ...
                >
                  <div className={cn(
                    ...
                    isActive ? "bg-primary text-white" : "bg-text-primary/5 text-text-secondary group-hover:text-text-primary"
                  )}>
                    <Icon className="w-6 h-6" />
                  </div>
                  <div className="flex-1">
                    <div className={cn("text-lg font-bold", isActive ? "text-text-primary" : "text-text-secondary")}>
                      {type.name}
                    </div>
                    <div className="text-xs text-text-secondary">View personalizations</div>
                  </div>
                  <ChevronRight className={cn("w-5 h-5 transition-transform", isActive ? "text-primary translate-x-1" : "text-text-primary/10")} />
                </button>
              );
            })}
          </div>
          ...
          <div className="lg:col-span-8 relative">
            ...
            >
              <div className="bg-background rounded-[24px] overflow-hidden border border-border-custom">
                {/* Device Header */}
                <div className="p-6 border-b border-border-custom flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-text-primary">Alex Rivera</div>
                      <div className="text-[10px] text-text-secondary uppercase tracking-widest font-bold">Identity Owner</div>
                    </div>
                  </div>
                  <div className="px-3 py-1 rounded-full bg-text-primary/5 border border-border-custom text-[10px] font-bold text-text-secondary flex items-center gap-2">
                ...
                {/* Profile Banner */}
                <div className={cn("h-32 bg-linear-to-r opacity-20", activeType.color, "to-transparent")} />
                ...
                <div className="p-8 -mt-16">
                  <div className="flex items-end justify-between mb-8">
                    <div className="p-4 glass rounded-2xl">
                      ...
                      <div className="text-xl font-bold text-text-primary">{activeType.title}</div>
                    </div>
                    <div className="text-xs text-text-secondary bg-text-primary/5 px-3 py-1.5 rounded-lg border border-border-custom">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    {activeType.content.map((item, idx) => (
                      <div key={idx} className="p-4 rounded-xl bg-text-primary/5 border border-border-custom">
                  ...
                  <div className="space-y-3">
                    <div className="h-2 w-full bg-text-primary/5 rounded-full overflow-hidden">
                      <motion.div 
                        ...
                        className={cn("h-full bg-linear-to-r", activeType.color, "to-transparent")} 
                      />
                    </div>
                    <div className="h-2 w-3/4 bg-text-primary/5 rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: '45%' }}
                  ...
                  <div className="mt-8 pt-8 border-t border-border-custom flex justify-between items-center">
                    ...
                    <button className={cn("px-6 py-2 rounded-xl text-xs font-bold text-white transition-all", "bg-linear-to-r", activeType.color, "to-primary/50")}>
                      Request Access
                    </button>
                  </div>
                </div>
