...
import { Shield, Zap, Layout, Share2, BarChart, Users, Cloud, Lock } from 'lucide-react';
...
const FeatureCard = ({ feature, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={cn(
        "relative group overflow-hidden glass-card p-8 min-h-[280px] flex flex-col justify-between hover:border-primary/50 transition-colors",
        feature.className
      )}
    >
      <div className={cn("absolute top-0 right-0 w-32 h-32 bg-linear-to-br opacity-0 group-hover:opacity-10 blur-3xl transition-opacity", feature.color, "to-transparent")} />
      
      {feature.visual}
      
      <div className="relative z-10">
        <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mb-6 bg-text-primary/5 border border-border-custom group-hover:bg-primary group-hover:text-white transition-all")}>
    ...
    <section id="features" className="py-32 bg-transparent relative">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <button className="px-6 py-3 border border-border-custom rounded-xl text-sm font-bold hover:bg-text-primary/5 transition-colors text-text-primary">
            View All Features
          </button>
        </div>
